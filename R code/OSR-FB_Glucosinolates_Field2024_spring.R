...
EVA_Gluco <- subset(EVA_Gluco, EVA_Gluco$Sampling_campain == "EVA_2_AF")
...
model <- lm(log(total+1) ~ bloc + variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
EVA_Gluco$log_total <- log(EVA_Gluco$total+1)

anova_table <- EVA_Gluco  %>% anova_test(log_total ~ bloc + variety*culture )
...
plot_total <- ggplot(emm, aes(x =response , y = variety, color = culture)) + 
...
EVA_Gluco$variety <- as.factor(EVA_Gluco$variety)
EVA_Gluco_glucoraphanin <- subset(EVA_Gluco, EVA_Gluco$Glucoraphanin< 3)

model <- lm(log(Glucoraphanin+1)~ bloc + variety*culture  , data=EVA_Gluco_glucoraphanin) #calculer le model, la covariable passe en premier 
...
EVA_Gluco_glucoraphanin$log_Glucoraphanin <- log(EVA_Gluco_glucoraphanin$Glucoraphanin+1)

anova_table <- EVA_Gluco_glucoraphanin  %>% anova_test(log_Glucoraphanin ~ bloc + variety*culture )
...
model <- lm(log(Glucoalyssin+1) ~ bloc + variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
anova_table <- EVA_Gluco  %>% anova_test(log_glucoalyssin ~ bloc + variety*culture )
...
model <- lm(log(Gluconapin+1) ~ bloc + variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
EVA_Gluco$log_Gluconapin <- log(EVA_Gluco$Gluconapin+1)
...
KW_table <- kruskal.test(Butyl.GS ~ variety, data = EVA_Gluco)
variety_p <- KW_table$p.value

KW_table <-kruskal.test(Butyl.GS ~ culture, data = EVA_Gluco)
...
model <- lm(log(Butyl.GS+1) ~ bloc + variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
EVA_Gluco  %>% anova_test(Butyl.GS ~ bloc + variety*culture )
...
model <- lm(log(Neoglucobrassicin+1) ~ bloc + variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
write.csv(anova_results, "~/working/Aboveground_characterization/3-Output/anova_results_EVA_2023_AF.csv" )
...
figure <- ggarrange(Weight, total, Butyl, Glucoraphanin, Glucobrassicin, Glucobrassicanapin, 
