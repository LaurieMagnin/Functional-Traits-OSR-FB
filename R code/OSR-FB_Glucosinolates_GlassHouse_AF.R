...
EVA_Gluco <- subset(EVA_Gluco, EVA_Gluco$Sampling_campain == "GH_AF")
...
model <- lm(log(total+1) ~  variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
anova_table <- EVA_Gluco  %>% anova_test(log_total ~  variety*culture )
anova_table
...
KW_table <- kruskal.test(Glucoraphanin ~ variety, data = EVA_Gluco)
variety_p <- KW_table$p.value

KW_table <-kruskal.test(Glucoraphanin ~ culture, data = EVA_Gluco)
...
model <- lm(log(Progoitrin.isomer+1) ~  variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
EVA_Gluco$log_Progoitrin <- log(EVA_Gluco$Progoitrin.isomer+1)

anova_table <- EVA_Gluco  %>% anova_test(log_Progoitrin ~  variety*culture )
anova_table
...
model <- lm(log(Gluconapin+1) ~ variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
KW_table <- kruskal.test(Glucobrassicin ~ variety, data = EVA_Gluco)
variety_p <- KW_table$p.value

KW_table <-kruskal.test(Glucobrassicin ~ culture, data = EVA_Gluco)
culture_p <- KW_table$p.value
...
model <- lm(log(Gluconasturtiin+1) ~ variety*culture  , data=EVA_Gluco) #calculer le model, la covariable passe en premier 
...
anova_table <- EVA_Gluco  %>% anova_test(log_Gluconasturtiin ~  variety*culture )
...
KW_table <- kruskal.test(Neoglucobrassicin ~ variety, data = EVA_Gluco)
variety_p <- KW_table$p.value

KW_table <-kruskal.test(Neoglucobrassicin ~ culture, data = EVA_Gluco)
culture_p <- KW_table$p.value
...
write_csv(anova_results, "~/working/Aboveground_characterization/3-Output/anova_results_GlassHouse_AF.csv" )
